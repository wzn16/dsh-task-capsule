...
import { PILOT_NS, PilotConfig, migrateNotifyFromDock } from './src/host-core.js'
...
export function apply(ctx) {
  ...
  // 自有 settings 命名空间（dsh-pilot）：通知行为配置持久化；
  // 注册完成后从 dsh-dock 的 notify 段只读迁移一次旧值（不写回 dsh-dock）。
  ctx.inject(['settings'], (sctx) => {
    sctx.settings.register(PILOT_NS, PilotConfig, {})
  ...
  console.log('[dsh-pilot] host half loaded; features: runstate, notify')
